...
export default function Dashboard() {
  ...
  const refresh = useCallback(async () => {
    const [t, sum, recs, ann] = await Promise.all([
      getTargets(),
      getDailySummary(currentDate),
      getRecords({ date: currentDate }),
  ...
  const openEditModal = async (record) => {
    ...
    setAutocomplete([])
    if (record.foodId) {
      const food = await getFoodById(record.foodId).catch(() => null)
  ...
  const handleFoodSearch = async (val) => {
    ...
    const results = await searchFoods(val).catch(() => [])
  ...
  const handleSave = async () => {
    ...
    if (isEditing) await updateRecord(form.recordId, data)
    else await addRecord(data)

    bsRecord.current.hide()
    const [newSummary, newRecords] = await Promise.all([
      getDailySummary(currentDate),
      getRecords({ date: currentDate })
    ])
