...
export default function AdminUsers() {
  ...
  const loadUsers = () => {
    const list = getStorage('ddas_users') || []
    setUsers(list)
  }

  const saveUsers = (updated) => {
    setStorage('ddas_users', updated)
    setUsers(updated)
  }
  ...
  const toggleActive = (userId) => {
    ...
    const updated = list.map(u =>
      u.userId === userId ? { ...u, isActive: !u.isActive, updatedAt: formatISO() } : u
    )
    saveUsers(updated)
  ...
  const handleDelete = () => {
    const list = getStorage('ddas_users') || []
    const updated = list.filter(u => u.userId !== pendingDeleteId)
    saveUsers(updated)
  ...
  const handleResetPassword = async () => {
    ...
    try {
      ...
      const list = getStorage('ddas_users') || []
      const updated = list.map(u =>
        u.userId === resetTarget.userId
          ? { ...u, passwordHash: hash, mustChangePassword: true, updatedAt: formatISO() }
          : u
      )
      saveUsers(updated)
