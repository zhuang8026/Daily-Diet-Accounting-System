...
import { getAllRecordsForAdmin, deleteRecord } from '@/assets/api/recordService'
...
export default function AdminRecords() {
  ...
  const loadRecords = () => {
    const result = getAllRecordsForAdmin({
      startDate: filters.startDate,
      endDate: filters.endDate,
      mealType: filters.mealType || undefined,
      userId: undefined
    })
    const filtered = filters.userEmail
      ? result.filter(r => r.userEmail?.includes(filters.userEmail))
      : result
    setRecords(filtered)
  }
  ...
  const openDelete = (userId, recordId) => {
    setPendingDelete({ userId, recordId })
    setShowDeleteModal(true)
  }

  const handleDelete = () => {
    const { userId, recordId } = pendingDelete
    const ok = deleteRecord(userId, recordId)
    if (ok) {
                ...
                <tr key={r.recordId}>
                  ...
                  <td>
                    <button
                      ...
                      onClick={() => openDelete(r.userId, r.recordId)}
                    >
                      刪除
                    </button>
                  </td>
