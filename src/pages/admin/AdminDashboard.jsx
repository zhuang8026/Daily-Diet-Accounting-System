...
import { getStorage } from '@/assets/api/storage'
import { getTodayISO, offsetDate } from '@/assets/api/utils'
...
const computeStats = () => {
  ...
  const weekRecords = allRecords.filter(
    r => r.recordDate >= weekStart && r.recordDate <= today
  )

  const foodCount = weekRecords.reduce((acc, r) => {
    acc[r.foodName] = (acc[r.foodName] || 0) + 1
    return acc
  }, {})
